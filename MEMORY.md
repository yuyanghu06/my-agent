...
- 2026-05-24 — **Shift / MicroAGI design guidelines source.** Before producing or editing ANY design for Shift or MicroAGI (Figma files, Figma Make, IG posts, print specs / merch specs, slide decks, marketing assets, etc.), open the brand design-guidelines Drive folder and read the relevant doc(s) first: https://drive.google.com/drive/folders/1tpOSzOgsgJQ9i76t7QKUcae4W9BBv_2C — list it via the google-* Drive MCP that has access (try personal1 first, fall back to others), then conform to logo usage, color, type, layout, and voice. Don't skip even for "small" tweaks — Yuyang treats brand consistency as load-bearing.
...
- 2026-05-27 — **Daemon launchd gotcha (TCC).** The `com.yuyang.agent` LaunchAgent's `StandardOutPath`/`StandardErrorPath` and `WorkingDirectory` must NOT live under `~/Documents` (TCC-protected). launchd's spawn-time file-open/chdir there gets denied → job exits **78 (EX_CONFIG)** *before any output*, crash-loops, no `/tmp/claude-agent.sock`, and "Cannot reach daemon" errors in spotlight. The spawned node process itself CAN read/write the repo fine (it uses absolute `CLAUDE_AGENT_ROOT`); only launchd's setup is blocked. Fix applied: log → `~/Library/Logs/com.yuyang.agent.log`, `WorkingDirectory` → `/Users/yuyang`. An OS update (macOS Tahoe) can reset this and re-break it. Recovery: `launchctl bootout gui/$(id -u)/com.yuyang.agent && launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.yuyang.agent.plist`, verify socket exists. Daemon's own rich log stays at `logs/daemon.log` (`npm run logs`).

## Decisions and commitments

- 2026-05-30 — **Spotlight frost is NATIVE, not CSS.** Reversed the earlier "CSS-only frost" assumption: CSS `backdrop-filter` cannot blur the desktop behind a transparent Tauri window (it only blurs in-page content), which is why the glass looked flat/see-through. Now using `tauri-plugin-liquid-glass` (crate + `tauri-plugin-liquid-glass-api` npm, both 0.1.6) → real macOS 26 `NSGlassEffectView` behind the transparent webview, `NSVisualEffectView` fallback on older OSes. Wired in `Cargo.toml` + `.plugin(tauri_plugin_liquid_glass::init())` in main.rs, `capabilities/default.json` (`liquid-glass:default`), and called via `setLiquidGlassEffect({ cornerRadius: 8 })` in main.ts + reply.ts. Removed the now-unused `window-vibrancy` dep. `#app`/`#reply-app` now only paint the `--glass-grad` tint + rim ON TOP; `backdrop-filter` kept ONLY on in-page overlays (#commands, #actions-menu, .toast, .sel-popover, .paste-overlay) where there IS page content behind them. Uses a private Apple API (fine for personal use, App Store risk). Known tradeoff: native glass fills the whole window, so the `menu-float` transparent-gap dropdown will show glass not desktop — fix later via a separate borderless window for the dropdown.

- 2026-05-30 — **Spotlight Figma design direction.** The "Orange Liquid Glass" Figma board (file P5vFz9LI4XHv0FeOClmbVK) was reskinned to a **neutral editorial liquid-glass** look: off-white canvas (#ECEAE5), white glass surfaces (~85% opacity + background blur + white rim highlight), charcoal ink (#1A1714), secondary gray (#6E6A64), minimal radii (8/4), generous label tracking. Orange accent was **dropped** in favor of graphite (#3A3631) solids + a **slate-blue gradient accent (#44557A → #93A4C2)** on CTAs/toggle/thumb/sweeps. Reference was fluid.glass + Apple liquid-glass material. The desktop app code (`spotlight/src/style.css` + `index.html`) was synced to this neutral/slate direction on 2026-05-30: orange fully removed, aperture brand mark is now a graphite ring (#3A3631), placeholder "Ask anything…", glass gradient `linear-gradient(159deg, rgba(252,253,255,0.86), rgba(235,238,244,0.66))` + 13px backdrop blur, slate-blue accent reserved for CTAs/toggles/status sweep/caret. Exact tokens pulled from Figma via get_design_context. (The iOS client `spotlight-ios/` has NOT been reskinned yet.)
